...
(
    id        	BIGINT PRIMARY KEY AUTO_INCREMENT,
	ten        	NVARCHAR(30),    ngay_tao   	TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    ngay_sua	TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    trang_thai 	ENUM('ACTIVE', 'INACTIVE') DEFAULT 'ACTIVE'
);
...
(
    ...
    loai_voucher   			ENUM('INVOICE','CUSTOMER'),
    ngay_tao   				TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    ngay_sua   				TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    trang_thai             	ENUM('ONGOING', 'EXPIRED', 'ENDING_SOON', 'UPCOMING', 'CANCELLED'),

	hinh_thuc_giam_gia_id   BIGINT,
    FOREIGN KEY (hinh_thuc_giam_gia_id) REFERENCES hinh_thuc_giam_gia (id)
);

CREATE TABLE voucher_chi_tiet
(
    id                     	BIGINT PRIMARY KEY AUTO_INCREMENT,
    so_lan_su_dung  		INT,
    ngay_tao   				TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    ngay_sua   				TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    trang_thai             	ENUM('ACTIVE','INACTIVE', 'DELETED'),
...
(
    ...
    trang_thai         	ENUM('PENDING', 'CONFIRMED', 'SHIPPING','PICKUP', 'CANCELLED', 'APPROVED'),
    voucher_id         BIGINT,
    tai_khoan_id       BIGINT,
    FOREIGN KEY (voucher_id) REFERENCES voucher (id),
    FOREIGN KEY (tai_khoan_id) REFERENCES tai_khoan (id)
...
INSERT INTO vai_tro (ten) VALUES 
('MANAGER'),
('EMPLOYEE'),
('CUSTOMER');

-- Thêm dữ liệu vào bảng hinh_thuc_giam_gia
INSERT INTO hinh_thuc_giam_gia (ten) VALUES 
('Giá Tiền'),
('Phân Trăm');
...
-- ALTER TABLE du_an_tot_nghiep.dia_chi
...
-- CHANGE COLUMN trang_thai_enum trang_thai ENUM('ACTIVtimelineE', 'INACTIVE');
